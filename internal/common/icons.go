...
// LeafIcon generates the laurel leaf SVG icon
func LeafIcon(laurel string) string {
	laurelEscaped := html.EscapeString(laurel)
	return fmt.Sprintf(`<svg xmlns="http://www.w3.org/2000/svg" width="90pt" height="90pt" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet">
<metadata>
Created by potrace 1.15, written by Peter Selinger 2001-2017
</metadata>
<g transform="translate(20.000000,60.000000) scale(0.00400000,-0.00400000)" fill="%s" stroke="none">
...
// GetNextRankBar generates the progress bar SVG for next rank
func GetNextRankBar(title string, percentage float64, color string) string {
	maxWidth := 80
	width := float64(maxWidth) * percentage
	...
	colorEscaped := html.EscapeString(color)

	return fmt.Sprintf(`
    <style>
    @keyframes %sRankAnimation {
      from {
        width: 0px;
      }
      to {
    ...
    #%s-rank-progress{
      animation: %sRankAnimation 1s forwards ease-in-out;
    }
    </style>
    <rect
      x="15"
    ...
    <rect
      ...
      fill="%s"
    />
  `, titleEscaped, width, titleEscaped, titleEscaped, maxWidth, colorEscaped, titleEscaped, colorEscaped)
}

// GetSmallTrophyIcon generates small trophy icons for double/triple ranks
func GetSmallTrophyIcon(icon string, color string, count int) string {
	leftXPosition := 7
	rightXPosition := 68
	colorEscaped := html.EscapeString(color)

	getIcon := func(x int) string {
		return fmt.Sprintf(`<svg x="%d" y="35" width="65" height="65" viewBox="0 0 30 30" fill="%s" xmlns="http://www.w3.org/2000/svg">
      %s
    </svg>`, x, colorEscaped, icon)
	}
...
// GetTrophyIcon generates the trophy icon SVG
func GetTrophyIcon(theme themes.Theme, rank Rank) string {
	rankStr := string(rank)
	rankEscaped := html.EscapeString(rankStr)

	var color string
	var rankColor string
	var backgroundIcon string
	var gradationColor string
	iconCircleEscaped := html.EscapeString(theme.IconCircle)

	if rank == RankSecret {
		rankColor = theme.SecretRankText
		gradationColor = fmt.Sprintf(`
    <stop offset="0%%" stop-color="%s"/>
    <stop offset="50%%" stop-color="%s"/>
    <stop offset="100%%" stop-color="%s"/>
    `, html.EscapeString(theme.SecretRank1), html.EscapeString(theme.SecretRank2), html.EscapeString(theme.SecretRank3))
	} else if len(rankStr) > 0 && rankStr[0] == 'S' {
		color = theme.SRankBase
		rankColor = theme.SRankText
		backgroundIcon = LeafIcon(theme.Laurel)
		gradationColor = fmt.Sprintf(`
    <stop offset="0%%" stop-color="%s"/>
    <stop offset="70%%" stop-color="%s"/>
    <stop offset="100%%" stop-color="%s"/>
    `, html.EscapeString(color), html.EscapeString(color), html.EscapeString(theme.SRankShadow))
	} else if len(rankStr) > 0 && rankStr[0] == 'A' {
		color = theme.ARankBase
		rankColor = theme.ARankText
		backgroundIcon = LeafIcon(theme.Laurel)
		gradationColor = fmt.Sprintf(`
    <stop offset="0%%" stop-color="%s"/>
    <stop offset="70%%" stop-color="%s"/>
    <stop offset="100%%" stop-color="%s"/>
    `, html.EscapeString(color), html.EscapeString(color), html.EscapeString(theme.ARankShadow))
	} else if rank == RankB {
		color = theme.BRankBase
		rankColor = theme.BRankText
		gradationColor = fmt.Sprintf(`
    <stop offset="0%%" stop-color="%s"/>
    <stop offset="70%%" stop-color="%s"/>
    <stop offset="100%%" stop-color="%s"/>
    `, html.EscapeString(color), html.EscapeString(color), html.EscapeString(theme.BRankShadow))
	} else {
		color = theme.DefaultRankBase
		rankColor = theme.DefaultRankText
		gradationColor = fmt.Sprintf(`
      <stop offset="0%%" stop-color="%s"/>
      <stop offset="50%%" stop-color="%s"/>
      <stop offset="100%%" stop-color="%s"/>
  `, html.EscapeString(color), html.EscapeString(color), html.EscapeString(theme.DefaultRankShadow))
	}

	rankFirstChar := "?"
	if len(rankStr) > 0 {
		rankFirstChar = string(rankStr[0])
	}
    ...
    <circle cx="8" cy="6" r="4" fill="%s" />
    <text x="6" y="8" font-family="Courier, Monospace" font-size="7" fill="%s">%s</text>
  `, iconCircleEscaped, html.EscapeString(rankColor), html.EscapeString(rankFirstChar))

	rankLength := len(rankStr)
	iconCount := rankLength - 1
	if iconCount < 0 {
		iconCount = 0
	}
	optionRankIcon := GetSmallTrophyIcon(icon, color, iconCount)
  ...
  `, backgroundIcon, optionRankIcon, rankEscaped, gradationColor, rankEscaped, icon)
}
